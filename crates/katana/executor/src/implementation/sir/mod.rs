...
use sir::state::BlockInfo;

use self::state::CachedState;
pub use self::utils::Error;
use crate::abstraction::{
    BlockExecutor, ExecutionOutput, ExecutionSimulationFlag, ExecutorFactory, ExecutorResult,
...
impl NativeExecutorFactory {
    ...
    pub fn new(cfg_env: CfgEnv, simulation_flags: SimulationFlag) -> Self {
        Self { cfg_env, simulation_flags }
    }
}
...
impl<'a> StarknetVMProcessor<'a> {
    pub fn new(
        ...
        let state = CachedState::new(StateProviderDb(state), contract_class_cache);
...
impl<'a> TransactionExecutor for StarknetVMProcessor<'a> {
    ...
    fn simulate(
        ...
        let state = CachedState::new(state, PermanentContractClassCache::default());

        let block_context = &self.block_context;
        let simulation_flag = SimulationFlag {
            skip_validate: flags.skip_validate,
            skip_fee_transfer: flags.skip_fee_transfer,
            ..self.simulation_flags
        };
    ...
    fn call(&self, call: EntryPointCall, initial_gas: u128) -> ExecutorResult<Vec<FieldElement>> {
        ...
        let res = utils::call(call, &state, block_context, initial_gas)?;
        ...
        let storage_updates: HashMap<ContractAddress, HashMap<StorageKey, StorageValue>> =
            state_diffs
                ...
                .map(|(k, v)| {
                    ...
                    let v = v
                        .into_iter()
                        .map(|(k, v)| (utils::to_felt(&k), utils::to_felt(&v)))
                        .collect();
                    (k, v)
                })
                .collect();
