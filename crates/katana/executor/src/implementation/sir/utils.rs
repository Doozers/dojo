...
use sir::state::{ExecutionResourcesManager, StateDiff};
    ...
    C: ContractClassCache + Send + Sync,
{
    ...
    let res = {
        let cached_state = &mut state.0.write().inner;
        match tx {
    ...
    match res {
        Ok(inner) => {
            ...
            let gas_used = calculate_tx_l1_gas_usage(&inner.actual_resources, &block_context)?;
            Ok(TransactionExecutionInfo { inner, gas_used })
        }

        Err(err) => Err(Error::TransactionError(err)),
    }
}
    ...
    C: ContractClassCache + Send + Sync,
{
    ...
    let state = &mut state.0.write().inner;
