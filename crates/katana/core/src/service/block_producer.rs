...
impl BlockProducer {
    ...
    pub fn interval(backend: Arc<Backend<NativeExecutorFactory>>, interval: u64) -> Self {
        Self {
    ...
    pub fn on_demand(backend: Arc<Backend<NativeExecutorFactory>>) -> Self {
        Self {
            inner: Arc::new(RwLock::new(BlockProducerMode::Interval(
                IntervalBlockProducer::new_no_mining(backend),
            ))),
        }
    }
    ...
    pub fn instant(backend: Arc<Backend<NativeExecutorFactory>>) -> Self {
        Self {
...
impl IntervalBlockProducer {
    pub fn new(backend: Arc<Backend<NativeExecutorFactory>>, interval: u64) -> Self {
        let interval = {
    ...
    pub fn new_no_mining(backend: Arc<Backend<NativeExecutorFactory>>) -> Self {
        let provider = backend.blockchain.provider();
    ...
    pub fn executor(&self) -> Option<PendingExecutor> {
        if let Some(executor) = &self.executor { Some(executor.clone()) } else { None }
    }
    ...
    fn do_mine(
        ...
        let transactions = transactions
            .into_iter()
            .filter_map(|(tx, rct)| if let Some(rct) = rct { Some((tx, rct)) } else { None })
            .collect::<Vec<_>>();
...
impl InstantBlockProducer {
    pub fn new(backend: Arc<Backend<NativeExecutorFactory>>) -> Self {
        Self {
            backend,
            block_mining: None,
            queued: VecDeque::default(),
